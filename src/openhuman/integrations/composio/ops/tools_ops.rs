...
/// connected toolkits, fetch v3 schemas for them, then re-apply OpenHuman's
/// curated whitelist and the user's scope preference. All of it is the module's
/// now, including the scope filter.
///
/// The filter is deliberately *not* mirrored here. Two places deciding what a
/// user permitted is how one of them ends up stale and permissive, and the
/// permissive one is the one that lets an action through.
