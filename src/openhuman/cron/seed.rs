...
/// Name used by the retired TinyPlace autopilot schedule. This migration
/// marker must remain until upgraded workspaces have had the persisted job
/// removed.
const RETIRED_TINYPLACE_AUTOPILOT_JOB_NAME: &str = "tinyplace_autopilot";
...
///
/// Runs on every core boot (and is idempotent) so existing installations cannot
/// keep executing an enabled TinyPlace autopilot row after upgrading.
pub fn prune_retired_jobs(config: &Config) -> Result<usize> {
    let existing = list_jobs(config)?;
    let stale_ids: Vec<String> = existing
        .iter()
        .filter(|job| job.name.as_deref() == Some(RETIRED_TINYPLACE_AUTOPILOT_JOB_NAME))
...
mod tests {
    ...
    fn constants_are_valid_identifiers() {
        ...
        assert!(!RETIRED_TINYPLACE_AUTOPILOT_JOB_NAME.is_empty());
    }
    ...
    fn startup_prune_removes_retired_tinyplace_autopilot_jobs() {
        ...
        add_agent_job_with_definition(
            &config,
            Some(RETIRED_TINYPLACE_AUTOPILOT_JOB_NAME.to_string()),
            Schedule::Cron {
                expr: "*/5 * * * *".to_string(),
                tz: None,
                active_hours: None,
