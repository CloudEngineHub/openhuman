...
const actions = (
  <div className="flex items-center gap-2">
    <div className="rounded-lg border border-line bg-surface p-0.5 text-sm">
...
createRoot(document.getElementById('root')!).render(
  <div className="h-screen bg-chrome p-3">
    <div className="flex h-full gap-3">
      <div className="w-60 shrink-0 rounded-2xl border border-line/40 p-2 text-xs text-content-muted">app sidebar</div>
      <div className="relative min-w-0 flex-1 overflow-hidden rounded-2xl bg-surface">
        <div className="h-full p-4">
          <SettingsTabbedPage title={title} description="Build this automation step by step, then save and run it." leading={<Back />} headerAction={actions} scrollable={false} bodyFullBleed>
            <div className="h-full w-full bg-[radial-gradient(circle,rgb(var(--content-faint))_1px,transparent_1px)] [background-size:16px_16px] opacity-60" />
          </SettingsTabbedPage>
        </div>
      </div>
    </div>
