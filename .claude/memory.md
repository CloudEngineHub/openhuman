...
- **`SYNC_PASS_MAX_ITEMS` (200) has five consumers** — the Sources-row loop (`composio_sync_budgeted`, ≤50 passes) plus four single-pass entry points: `periodic.rs`, `memory/ops/sync.rs`, `bus_part_02.rs` (`connection_created` initial sync), `slack/rpc.rs`. One pass is one `AcceptSourceItems` call that embeds each document in turn (~1.7 s each) against the 15-min `module_call_slow!` deadline; 500 tripped it.
